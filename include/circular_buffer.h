...
size_t CircularBuffer<T>::write(const T *data, size_t count)
{
    ...
    size_t items_to_write = std::min(count, capacity - size_);
...
size_t CircularBuffer<T>::read(T *data, size_t count, bool del)
{
    ...
    size_t items_to_read = std::min(count, size_);
    size_t bytes_to_read = items_to_read*sizeof(T);

    // Read in a single step
    if (bytes_to_read <= capacity_*sizeof(T) - beg_index_)
    {
        memcpy(data, (void*)data_ + beg_index_, bytes_to_read);
        if (del) {
            beg_index_ += bytes_to_read;
            if (beg_index_ == capacity_*sizeof(T)) beg_index_ = 0;
        }
    }
    // Read in two steps
    else
    {
        size_t size_1 = capacity_*sizeof(T) - beg_index_;
